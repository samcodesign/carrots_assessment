...
Conversion rate (per variant) = \(\frac{\text{users with at least one }payment\_success}{\text{users with any event in that variant}}\), distinct by \texttt{revenue\_cat\_id}. All exposed users are the denominator; a payment\_success event signals conversion.
...
\[
\text{net\_revenue} = \text{price\_in\_usd} \times (1 - \text{tax\_percentage} - \text{commission\_percentage}).
\]
Using this:
\begin{itemize}
    \item Total net revenue (per variant) = sum of \texttt{net\_revenue} for subscriptions attached to that variant.
    \item ARPU = total net revenue \(/\) total users in the variant.
    \item ARPPU = total net revenue \(/\) paying users in the variant.
\end{itemize}
...
\begin{itemize}
    \item Retention 1+ = \% of paying users with \(\text{max\_renewal} \ge 1\)
    \item Retention 2+ = \% with \(\text{max\_renewal} \ge 2\)
    \item Retention 3+ = \% with \(\text{max\_renewal} \ge 3\)
\end{itemize}
